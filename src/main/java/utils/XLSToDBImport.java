...
public class XLSToDBImport {
    ...
    private ArrayList<String> announcesColumn = new ArrayList<>();
    ...
    public void startImport(ArrayList<ArrayList<String>> allImportXLSContent, ObservableList<String> allProducts) {
        ...
        try {
            allCompareDetails.stream().forEach((compareDetail) -> {
                ...
                case ("Краткое описание продукта"):
                    ...
                    break;
                case ("Анонс продукта"):
                    announcesColumn.addAll(getColumnByHeader(allImportXLSContent, compareDetail.get(0)));
                    for (int i = 0; i < announcesColumn.size(); i++) {
                        updateProducts("anons", announcesColumn.get(i), titlesColumn.get(i));
                    }
                    break;
